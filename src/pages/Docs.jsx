...
const CodeBlock = ({ code, fileName }) => {
    ...
    return (
        <div className='lg:w-[50%] my-5 border border-muted rounded-md mx-auto lg:mx-0' >
            <div className='px-5 py-3 flex justify-between items-center font-body bg-muted/20 text-muted text-sm md:text-base' >
                ...
                <button
                    ...
                    className={` px-3 py-1 rounded-md cursor-pointer hover:bg-muted/40 ${copied ? `bg-lime` : `bg-muted/20`}`}>{copied ? "Copied ✓" : "Copy"}</button>
            </div>
            <pre className='text-[#878787] px-8 py-4 text-sm md:text-base'>
                {code}
            </pre>
        </div>
    )
...
const Steps = ({ step }) => {
    return (
        <div className='mt-10'>
            {step.map((item, index) => (
                <div key={index} className="w-96 border border-muted mt-5 text-offwhite py-6 px-4 flex items-center justify-between rounded-lg">
    ...
    return (
        <div className='w-full min-h-screen' >
            ...
            <div className='w-full h-full lg:flex lg:min-h-screen' >
                ...
                {/* Sidebar */}
                <div className=' w-full lg:w-[18%] max-md:border-b lg:border-r border-muted py-10 flex flex-col items-center ' >
                    <h2 className=' text-2xl md:text-3xl lg:text-2xl text-offwhite font-heading font-extrabold lg:text-center mb-5'>Contents</h2>
                    {
                        sections.map((section, i) => (
                            <button
                                ...
                                className={` text-sm md:text-base px-5 py-2 rounded-lg mt-4 md:mt:6 lg:mt-8 font-body hover:bg-muted/40 hover:outline-0 hover:border-lime/50 w-fit transition-colors outline-none cursor-pointer ${selectedSection === section.id ? `bg-muted/40` : ""} `}>
                ...
                <div className='w-full px-5 lg:p-10' >
                    ...
                    {currentSection.content.blocks.map((block, i) => {
                        if (block.type === "text") return <p className='text-sm md:text-base font-body text-muted' >{block.content}</p>
